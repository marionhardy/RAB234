...
counts <- as.data.frame(read.csv("./data/RAB147_counts.csv", row.names = 1))
coldata = read.csv("./data/coldata.csv")
counts = counts[,-1]

### NON NUCLEOFECTED cells -----------------------------------------------------
# Going for the simplest design and comparing Nonuc Trpn vs Trpp
...
## Trpp vs Trpn for nonuc cells treatment --------------------------------------
...
write.csv(res_tbl,"./data_output/Nonuc_Trpp_vs_Trpn/res_tbl.csv", row.names = T)
...
write.xlsx(list_of_datasets, file = "./data_output/Nonuc_Trpp_vs_Trpn/RAB234_sign_genes.xlsx")
...
ggsave("./figures/Nonuc_Trpp_vs_Trpn/volcanoplot.png", last_plot(), device = png, dpi= 500, width = 8, height = 6)

# save the tibbles etc

saveRDS(dds, file = "./data_output/Nonuc_Trpp_vs_Trpn/dds.rds")
saveRDS(res_tbl, file = "./data_output/Nonuc_Trpp_vs_Trpn/res_tbl.rds")
write_csv(res_tbl, file = "./data_output/Nonuc_Trpp_vs_Trpn/res_tbl.csv")
res_tbl1 <-  res_tbl %>% filter(!is.na(ENTREZID), padj<0.05)
saveRDS(res_tbl1, file = "./data_output/Nonuc_Trpp_vs_Trpn/res_tbl_signif.rds")
write_csv(res_tbl1, file = "./data_output/Nonuc_Trpp_vs_Trpn/res_tbl_signif.csv")

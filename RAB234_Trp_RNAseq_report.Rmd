---
title: "RAB147_Trp_RNAseq_report"
author: "Marion Hardy"
date: "2023-05-31"
output: 
  html_document:
...
res_tbl <- read_csv("./data_output/Scr_Trpn_vs_Trpp/res_tbl.csv")
diff <- read_csv("./data_output/Scr_Trpn_vs_Trpp/res_tbl_signif.csv")
...
# res_tbl1 =
#   res_tbl1 %>% filter(abs(log2FoldChange)>= 0.5)
...
dotplot(gsea_results_hall, x = "NES", showCategory = 30, font.size = 8)+ 
  ggtitle("GSEA genetic perturbations")
...
## GSEA analysis abs(logFc)

```{r}

ordered_genes_fc <- abs(res_tbl1$log2FoldChange)
names(ordered_genes_fc) <- res_tbl1$gene
ordered_genes_fc <- sort(ordered_genes_fc, decreasing = T)
...
# dotplot(gsea_results_hall, x = "NES", showCategory = 30)+ ggtitle("GSEA pval Hallmarks")
# 
# gsea_results_hall %>% 
#   as_tibble() %>% 
#   select(1:9) %>%

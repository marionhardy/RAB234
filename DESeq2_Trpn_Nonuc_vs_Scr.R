...
### SCRAMBLE cells -------------------------------------------------------------
# Going for the simplest design and comparing Nonuc to Scr in Trpn

counts = counts %>%
  dplyr::select(starts_with(c("Nonuc_Trpn","Scr_Trpn")))

coldata = coldata %>% filter(condition=="Trpn", celltype %in%c("Nonuc","Scr"))
...
write.csv(res_tbl,"./data_output/Trpn_Nonuc_vs_Scr/res_tbl.csv", row.names = T)
...
write.xlsx(list_of_datasets, file = "./data_output/Trpn_Nonuc_vs_Scr/RAB234_sign_genes.xlsx")
...
res_tbl %>%
  ...
  geom_text_repel(max.overlaps = 20,
                  ...
                  fontface = "italic",
                  size = 3)+
  labs(title = "Nonuc vs scramble cells in Trpn medium")+
  theme_bw()

ggsave("./figures/Trpn_Nonuc_vs_Scr/volcanoplot.png", last_plot(), device = png, dpi= 500, width = 8, height = 6)

# save the tibbles etc

saveRDS(dds, file = "./data_output/Trpn_Nonuc_vs_Scr/dds.rds")
saveRDS(res_tbl, file = "./data_output/Trpn_Nonuc_vs_Scr/res_tbl.rds")
write_csv(res_tbl, file = "./data_output/Trpn_Nonuc_vs_Scr/res_tbl.csv")
res_tbl1 <-  res_tbl %>% filter(!is.na(ENTREZID), padj<0.05)
saveRDS(res_tbl1, file = "./data_output/Trpn_Nonuc_vs_Scr/res_tbl_signif.rds")
write_csv(res_tbl1, file = "./data_output/Trpn_Nonuc_vs_Scr/res_tbl_signif.csv")
